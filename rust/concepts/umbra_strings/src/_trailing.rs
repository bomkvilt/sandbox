...
impl From<&[u8]> for ArcDynBytes {
    fn from(bytes: &[u8]) -> Self {
        ...
        let layout = arc_data_layout(bytes.len());
...
unsafe impl OwnedBytes for ArcDynBytes {
    unsafe fn drop(&self, len: usize) {
        if len > 0 {
            ...
            if (*data).count.fetch_sub(1, atomic::Ordering::Acquire) == 1 {
                // NOTE: the dynamic part of the `data` is alreadt counted by the `layout`
                unsafe { std::alloc::dealloc(data.cast::<u8>(), arc_data_layout(len)) };
            }
        }
    }
}
...
fn arc_data_layout(len: usize) -> Layout {
